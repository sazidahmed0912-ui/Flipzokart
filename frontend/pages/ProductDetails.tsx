            ...
            {product.variants?.map((variant, vIdx) => {
              ...
              return (
                <div key={vIdx} className="mt-6">
                  <h3 className="text-sm font-semibold text-gray-900 mb-3">{variant.name}</h3>
                    ...
                    {variant.options.map((option, oIdx) => {
                      ...
                      if (matchesColor) {
                        return (
                          <button
                            ...
                            className={`w-8 h-8 rounded-full ${getColorClass(option)} border-2 ${isActive ? 'border-gray-800 ring-2 ring-offset-2 ring-gray-800' : 'border-gray-300'}`}
                            title={option}
                          />
                        );
                      }
                      ...
                      return (
                        <button
                          key={oIdx}
                          onClick={() => handleVariantSelect(variant.name, option)}
                          className={`px-5 py-2 rounded-lg border-2 text-sm font-medium ${isActive
